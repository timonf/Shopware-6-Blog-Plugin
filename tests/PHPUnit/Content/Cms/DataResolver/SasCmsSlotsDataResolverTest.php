...
class SasCmsSlotsDataResolverTest extends TestCase
{
     ...
     * @param bool $hasCmsSections
     * @param int $setDataExpectedCalls
     * @param int $resolveExpectedCalls
    ...
    public function testResolve(
        ...
        $this->decorated
            ->expects($this->exactly($resolveExpectedCalls))
            ->method('resolve')
    ...
    private function createCmsPageSections(): CmsSectionCollection
    {
        ...
        $cmsSections->expects($this->atLeast(2))->method('getBlocks')->willReturn($cmsBlocks);
     ...
     *
     * @param CmsSectionCollection|null $cmsSections
     ...
     * @param BlogEntriesEntity|null $blog
    ...
    private function createCmsSlot(string $slotType, int $expectedCalls, ?BlogEntriesEntity $blog): MockObject
    {
        ...
        $blogDetailSlot
            ...
            ->method('setData')
            ->with($this->isInstanceOf(BlogEntriesEntity::class));
