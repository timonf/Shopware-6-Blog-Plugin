...
class BlogPageLoaderTest extends TestCase
{
    ...
    /**
     * @param string|null $requestArticleId
     * @param bool $hasBlogEntry
     * @param string|null $detailCmsPageId
     * @param bool $hasCmsPage
     * @param int $expectedDispatches
    ...
    public function testLoadData(
        ...
        if ($detailCmsPageId) {
            $this->systemConfigService->expects($this->once())->method('get')->willReturn($detailCmsPageId);
        }

        if ($hasCmsPage) {
            $cmsPageSearchResults = $this->createCmsPageLoaderResult($detailCmsPageId);
            $this->cmsPageLoader->expects($this->once())->method('load')->willReturn($cmsPageSearchResults);
        }
        ...
        $this->eventDispatcher->expects($this->exactly($expectedDispatches))->method('dispatch');
     ...
     * @param array|null $metaInformation
    ...
    private function createBlogSearchResult(
        ...
        if ($hasBlogEntry) {
            $blogEntry = $this->createConfiguredMock(BlogEntriesEntity::class, [
                ...
                'getAuthor' => $this->createConfiguredMock(BlogAuthorEntity::class, [
